...
from mothernet.evaluation.baselines.nam_custom.types import Config


def cross_entropy(logits: torch.Tensor, targets: torch.Tensor, weights: torch.Tensor) -> torch.Tensor:
...
def penalized_loss(config: Config, logits: torch.Tensor, targets: torch.Tensor, weights: torch.tensor,
                   fnn_out: torch.Tensor, model: nn.Module, n_classes: int) -> torch.Tensor:
    ...
    if config.regression:
        ...
        #loss_func = bce_loss if n_classes == 2 else cross_entropy
